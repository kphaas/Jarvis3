...
def _read_pg_password():
    import os
    sf = os.path.expanduser('~/jarvis/.secrets')
    for line in open(sf):
        if line.startswith('POSTGRES_PASSWORD='):
            return line.strip().split('=',1)[1]
    return 'jarvisdb'

DB_DSN = f'postgresql://jarvis:{_read_pg_password()}@localhost:5432/jarvis'
...
def _query_recent_summaries() -> List[Dict[str, Any]]:
    ...
    with _get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                """
                SELECT id, item_id, summary, tags, processed_at
...
def _query_routing_yesterday() -> Dict[str, Any]:
    ...
    with _get_conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """
                SELECT provider_chosen AS provider, COUNT(*) AS cnt
